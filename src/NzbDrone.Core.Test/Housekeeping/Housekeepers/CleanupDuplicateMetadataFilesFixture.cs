...
{
    [TestFixture]
    public class CleanupDuplicateMetadataFilesFixture : DbTest<CleanupDuplicateExtraFiles, ExtraFile>
    {
        [Test]
        public void should_not_delete_metadata_files_when_they_are_for_the_same_series_but_different_consumers()
        {
            var files = Builder<ExtraFile>.CreateListOfSize(2)
                                          .All()
                                          .With(m => m.MetadataType = MetadataType.SeriesMetadata)
                                          .With(m => m.SeriesId = 1)
                                          .BuildListOfNew();
        ...
        public void should_not_delete_metadata_files_for_different_series()
        {
            var files = Builder<ExtraFile>.CreateListOfSize(2)
                                          .All()
                                          .With(m => m.MetadataType = MetadataType.SeriesMetadata)
                                          .With(m => m.MetadataConsumer = "XbmcMetadata")
                                          .BuildListOfNew();
        ...
        {
            var file = Builder<ExtraFile>.CreateNew()
                                         .BuildNew();
